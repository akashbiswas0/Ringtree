...
export class RingSecret implements SecretProvider {
  ...
  unlocked() {
    return this.ready() && hasSessionPassword();
  }
  async withSecret<T>(fn: (secret: string) => Promise<T>): Promise<T> {
    ...
    const bytes = await withSessionPassword((password) =>
      ringTransform("decrypt", ciphertext, password, this.keyName),
    ).catch((error: Error) => {
      if (error.message.includes("password was not accepted"))
        throw new Error("KEY_RING_PASSWORD_REJECTED");
      if (error.message.includes("OS keychain"))
        throw new Error("KEY_RING_KEYCHAIN_UNAVAILABLE");
